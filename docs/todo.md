...
- [~] 카테고리 CRUD/비활성화와 색상/정렬, 실제 API 기반 화면 구현. 실제 DB 검증은 계속 구현.
- [~] 회의록 작성, 수정, 숨김, 목록, 상세, 필터와 실제 API 기반 화면 구현. 실제 DB 검증은 계속 구현.
...
- [~] Render `PORT`, production CORS origin, HTTPS 세션 쿠키 설정. 실제 브라우저의 cross-origin 세션·제3자 쿠키 동작과 CSRF·보안 헤더 점검은 계속 필요.
