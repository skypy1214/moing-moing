...
- 공개 게스트 진입은 제공하지 않으며, 모든 API 요청에는 로그인 세션이 필요하다. 발급된 계정은 일반 운영 기능을 조회·변경할 수 있고, `ADMIN`만 운영 계정·작업 이력 관리 API(`/api/v1/admin/**`)와 게시판 카테고리 관리 API에 접근한다.
...
- 관리자 전용 작업 이력은 작업자 로그인 ID와 한국 시간대의 시작일·종료일로 필터하고, 최신순 서버 페이지네이션으로 조회한다. 날짜 종료 조건은 선택일 전체를 포함하는 다음 날 0시 미만으로 해석한다.
...
Render Free 인스턴스의 재기동 시간을 줄이기 위해 운영 프로필은 지연 초기화를 사용한다. 대신 서버가 ready 상태가 되기 전에 DispatcherServlet과 로그인·회원 목록에 필요한 계정·회원·출석 조회 경로를 내부 warm-up으로 준비한다. 이 비용을 첫 운영자의 로그인과 첫 회원 목록 조회로 넘기지 않는다.
